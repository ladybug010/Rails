class DepositsController < ApplicationController
  ...
  def new
    ...
    logger.debug ("Adding deposit for account #{@account.number}")
    @deposit = Deposit.new
  ...
  def create
    @account = Account.find(params[:account_id])
    @deposit = @account.deposits.build(params[:deposit])
   
  
    #@deposit = Account.find(params[:account_id]).deposits.build(params[:deposit])
    respond_to do |format|
      if @deposit.save
